...
홈의 대표 비주얼과 빈 여행 기록은 도시와 무관한 공통 캐릭터/색상을 사용합니다. 항구 일러스트는 `MockCatalog`의 자갈치 `coverImage`에만 연결합니다. 새 코스는 `MockCatalog.courses`에 추가하면 홈과 도시 탐색에 표시됩니다. 이미지가 없거나 번들에서 찾지 못하면 공통 브랜드 비주얼을 표시합니다. 실제로 제공하는 샘플 코스는 여전히 자갈치 하나이며, 다른 도시의 콘텐츠를 임의로 만들지 않았습니다.

지도·완료·플레이어의 제목과 도시명은 선택한 코스에서 읽습니다. 이야기 상세 내비게이션도 코스를 함께 전달하고, 다음 이야기는 같은 코스 안에서만 찾습니다. `CourseContentTests`는 별도 서울 테스트 데이터를 사용해 코스 간 이야기 혼입을 검사합니다.
